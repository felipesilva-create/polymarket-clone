...
export default function TradeModal({ market, isOpen, onClose }: TradeModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#141419] rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto relative border border-[#252530]">
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 text-gray-400 hover:text-white bg-black/40 rounded-full p-1.5"
        >
        ...
        {market.youtubeId && (
          <CameraFeed
            youtubeId={market.youtubeId}
            youtubeIds={market.youtubeIds}
            cameraName={`${market.cameraName} - ${market.cameraLocation}`}
          />
        )}
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-4">
            <h2 className="text-xl font-bold text-white pr-8">{market.question}</h2>
            {market.duration && (
              <span className="shrink-0 text-xs font-bold text-amber-400 bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-500/20">
                {market.duration}
              </span>
            )}
          </div>
          ...
          {/* Outcome Selection */}
          <div className="mb-4">
            <label className="text-sm text-gray-400 mb-2 block">Sua previsao</label>
          ...
          <div className="bg-[#1a1a22] rounded-xl p-4 mb-4 border border-[#252530]">
            ...
            {mode === "buy" && (
              <div className="flex justify-between text-sm mb-2">
                <span className="text-gray-400">Lucro potencial (se acertar)</span>
